...
export function Duaa() {
  ...
  return (
    <div className="relative isolate flex w-full flex-col bg-obsidian">
      {/* Hero banner — background scoped to this section only */}
      <section className="relative isolate flex min-h-[88vh] w-full flex-col justify-end overflow-hidden">
        <div className="pointer-events-none absolute inset-0 z-0" aria-hidden>
          <motion.div
            initial={{ scale: 1.06, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 1.4, ease }}
            className="absolute inset-0 bg-cover bg-[left_30%_center] bg-no-repeat brightness-[1.12]"
          ...
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_18%,rgba(244,114,182,0.14),transparent_48%)]" />
          <div className="noise-overlay absolute inset-0 opacity-[0.5]" />
        </div>

        <div className="relative z-10 px-5 pb-20 pt-28 md:px-8 lg:px-10 lg:pb-28">
          <motion.div
          ...
          >
            ...
            <h1 className="mt-8">
              <img
                src={duaaLogo}
                alt="Duaa"
                className="block h-auto w-auto max-h-[5rem] md:max-h-[4rem] lg:max-h-[4.5rem]"
              />
            </h1>
